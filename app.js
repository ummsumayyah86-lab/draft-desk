...
(function () {
  ...
  function boot() {
    ...
    // Soft stubs so missing full UI buttons do not throw
    ['btnDrafts','btnNew','btnExport','btnExpand','btnCollapse','btnCloseDrafts','btnCreateNamed','btnCloseExport','btnDlMd','btnDlJson','btnDlHtml','articleFile'].forEach(function (id) {
      var n = el(id);
      if (!n) return;
      if (id === 'btnExpand') n.addEventListener('click', function () { document.querySelectorAll('details.lens').forEach(function (d) { d.open = true; }); });
      if (id === 'btnCollapse') n.addEventListener('click', function () { document.querySelectorAll('details.lens').forEach(function (d) { d.open = false; }); });
      if (id === 'btnNew') n.addEventListener('click', function () {
        saveNow();
      ...
      if (id === 'btnExport' || id === 'btnDrafts') n.addEventListener('click', function () {
        alert('Full Export/Drafts list lands with the complete app.js upload. Your marks still autosave on this device.');
      });
    });
  }
